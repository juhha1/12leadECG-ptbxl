...
class Block(nn.Module):
    def __init__(self, 
        ...
        if out_filters != in_filters or stride != 1 or stride != (1,1):
            skip = [nn.Conv1d(in_filters, out_filters, 1, stride = stride, bias = False)]
            if bn:
                skip.append(nn.BatchNorm1d(out_filters))
            self.skip = nn.Sequential(*skip)
        else:
            self.skip = None
        padding_size = (kernel_size // 2, kernel_size // 2 - (kernel_size - 1) % 2)
        
        rep = []
        filters = in_filters
        if grow_first:
            rep.append(self.act_fn())
            rep.append(nn.ReplicationPad1d(padding_size))
            rep.append(SeparableConv1d(in_filters, out_filters, kernel_size, stride = 1, padding = 0, bias = False))
            if bn:
        ...
        for i in range(reps-1):
            rep.append(self.act_fn())
            rep.append(nn.ReplicationPad1d(padding_size))
            rep.append(SeparableConv1d(filters, filters, kernel_size, stride = 1, padding = 0, bias = False))
            if bn:
        ...
        if stride != (1,1) or stride != 1:
            rep.append(nn.ReplicationPad1d(padding_size))
            rep.append(nn.MaxPool1d(kernel_size, stride, 0))
...
class Xception(nn.Module):
    ...
    def __init__(self,
        ...
        if act_fn == 'relu':
            ...
            self.act_fn = nn.GELU
        ### ENTRY FLOW
        self.conv1 = nn.Conv1d(input_channel, 32, list_kernel_size[0], list_strides[0], 0, bias = False)
        self.bn1 = nn.BatchNorm1d(32)
        self.act1 = self.act_fn()
        
        self.conv2 = nn.Conv1d(32, 64, list_kernel_size[1], 1, bias = False)
        ...
        self.conv3 = SeparableConv1d(1024, 1536, list_kernel_size[14], 1, 1)
        self.bn3 = nn.BatchNorm1d(1536)
        self.act3 = self.act_fn()
        
        self.conv4 = SeparableConv1d(1536, 2048, list_kernel_size[15], 1, 1)
